...
hindi_data %>%
  ...
  summarise(
    ...
    mean_gd = mean(IA_DWELL_TIME, na.rm = TRUE),
    sd_gd = sd(IA_DWELL_TIME, na.rm = TRUE)
  ) %>% 
  gt::gt() %>%
...
ggplot(hindi_data, aes(x = cond_name, y = IA_FIRST_FIXATION_DURATION, fill = cond_name)) +
...
## Raincloud Plot - Hindi Data (Gaze duration) {.scrollable}
...
ggplot(hindi_data, aes(x = cond_name, y = IA_DWELL_TIME, fill = cond_name)) +
  ...
  # Axis lables, theme
  labs(
    title = "Raincloud Plot: Gaze duration",
    x = "Condition",
    y = "Gaze duration (ms)"
  ) +
  theme_minimal() +
...
- We seem to have a few 0s in the dwell time data
- There are also some very long dwell times. 
- We should eliminate very long times, maybe with a cutoff at 1200

```{r remove-zeros-dwell-time}
#| echo: true
# Remove 0 and very long dwell time values by setting them to NA
hindi_data_clean <- hindi_data %>%
  mutate(IA_DWELL_TIME  = if_else(condition = IA_DWELL_TIME > 0 & IA_DWELL_TIME < 1200,
                                 true = IA_DWELL_TIME, 
                                 false = NA))
```

## Frequency effect in GD {.scrollable}

```{r hindi-raincloud-frequency-as-fill-gd}
#| echo: true
#| output-location: slide
#| fig.width: 10
#| fig.height: 6
...
## Hind data (factorial)
- Setting up factors
- Research Question: How do word type (romance vs. traditional) and frequency (low vs. high) affect reading time?
...
model_hindi_treatment <- lmer(IA_FIRST_FIXATION_DURATION ~ writing_system * frequency + 
                     (1|RECORDING_SESSION_LABEL) + (1|stim_num), 
